...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_MOSI_PIN,       default=27):      pins.internal_gpio_output_pin_schema,
    cv.Optional(CONF_MISO_PIN,       default=19):      pins.internal_gpio_input_pin_schema,
    cv.Optional(CONF_CLK_PIN,        default=5):      pins.internal_gpio_output_pin_schema,
    cv.Optional(CONF_CS_PIN,         default=18):       pins.internal_gpio_output_pin_schema,
    cv.Optional(CONF_GDO0_PIN,       default=26):       pins.internal_gpio_input_pin_schema,
    cv.Optional(CONF_GDO2_PIN,       default=4):       pins.internal_gpio_input_pin_schema,
    cv.Optional(CONF_LED_PIN,        default=25):      pins.gpio_output_pin_schema,
...
async def to_code(config):
    ...
    cg.add_library("jgromes/RadioLib", "7.0.0")
    cg.add_platformio_option("lib_ldf_mode", "chain")
    cg.add_platformio_option("board_build.extra_flags", "-DRADIOLIB_GODMODE")
